...
window.onscroll = function () {
  ...
  var second = document.getElementById('apply-wrapper-inner');
  second.setAttribute("style", 'left: -' + scroll + 'px');
};

function mouseLocation(onMove, listeningElementId) {
  if (listeningElementId) document.getElementById(listeningElementId).onmousemove = handleMouseMove;else document.onmousemove = handleMouseMove;
...
var statusBlocks = document.getElementsByClassName('card-body-hover');
Array.prototype.forEach.call(statusBlocks, function (status) {
  status.onmouseover = function (event) {
    mouseLocation(function (x, y) {
      var statusX = $('#apply-status-info').position().left;
      var statusY = ($('#apply-status-info').position().top + $('#apply-status-info').outerHeight(true)) / 10;
      var statusPath = $('#status-path');
      var curveWidth = status.offsetWidth * 2;
      var endX = $('#apply-status-info').width();
...
document.getElementById('apply-status-info').onmouseout = function (event) {
  updateStatusPath();
};

var updateStatusPath = function updateStatusPath() {
  var statusX = $('#apply-status-info').position().left;
  var statusY = ($('#apply-status-info').position().top + $('#apply-status-info').outerHeight(true)) / 10;
  var statusPath = $('#status-path');
  var endX = $('#apply-status-info').width();
  var centerX = $('.current-status').position().left + $('.current-status').outerWidth(true) / 2;
  var curveWidth = $('.status-block').outerWidth(true) * 2;
...
var createPath = function createPath(startX, startY, centerX, endX, endY, curveWidth) {
  for (var _len = arguments.length, pathEls = Array(_len > 6 ? _len - 6 : 0), _key = 6; _key < _len; _key++) {
    pathEls[_key - 6] = arguments[_key];
  }

  startY = Math.round(startY);
  var pathStart = 'M' + startX + ',' + startY;
  var pathEnd = 'L' + endX + ',' + endY;

  curveWidth = 6 * Math.round(curveWidth / 6);
  centerX = 6 * Math.round(centerX / 6);
  var curveStartX = Math.round(centerX - curveWidth / 2);
  var pathCoordinates = {
    0: 'L' + curveStartX + ',' + startY
  };

  var incrementX = 3 * Math.round(curveWidth / 2 / 3 / 3);
  var unit = incrementX / 3;
  var correctPath = '';

  var currentX = curveStartX;
  var currentY = startY;
  var pointCount = 1;
  ...
  //create curved path
  for (var point in pathCoordinates) {
    if (!point) correctPath = pathCoordinates[point];else correctPath += ' ' + pathCoordinates[point];
  }
  ...
  pathEls.forEach(function (pathEl) {
    return pathEl.attr('d', correctPath);
  });
};
...
Array.prototype.forEach.call(panels, function (panel) {
  return panel.setAttribute("style", 'width: ' + width + 'px;');
});
...
applyButton.onclick = function () {
  ...
  var processWrapperHeight = $('#curve').position().top + $('#curve').outerHeight(true);
  var processWrapper = document.getElementById('apply-process-wrapper');
  ...
  var startX = $('#apply-wrapper-inner').position().left;
  var startY = $('#apply-wrapper-inner').position().top + 10;
  var panelWrapperCenterX = $('#panel-wrapper').position().left + $('#panel-wrapper').width() / 2;
  var endX = $('#panel-wrapper').width();
  var curveWidth = $('#panel-wrapper').width();
  var trail = $('#test');
  createPath(startX, startY, panelWrapperCenterX, endX, startY, curveWidth, trail);

  var innerWrapper = document.getElementById('apply-wrapper-inner');
  ...
  var applyCardWrapper = document.getElementById('apply-card-wrapper');
...
submitButton.onclick = function (evt) {
  ...
  var vPanel = document.getElementById('verification');
  ...
  var verifyCardWrapper = document.getElementById('verify-card-wrapper');
...
var createTimer = function createTimer(timerLength) {
  ...
  var timer = setInterval(function () {
    var totalTime = timerLength;
    var elapsedTime = Date.now() - startTime;
    var remainingTime = 1000 * Math.round((totalTime - elapsedTime) / 1000);
    remainingTime = remainingTime / 1000;
    var secs = remainingTime % 60;
    var mins = Math.trunc(remainingTime / 60) % 60;
    var hours = Math.trunc(remainingTime / 3600) % 24;
    var days = Math.trunc(remainingTime / 3600 / 24);

    var timeStrArr = [hours, mins, secs].map(function (time) {
      return (time < 10 ? '0' : '') + time;
    });

    var remainingTimeStr = (days < 10 ? '0' : '') + days + 'D ' + timeStrArr.join(':');

    if (remainingTime >= 0) {
      var verifyTimer = document.getElementById('verify-timer');
      if (remainingTime === 0) verifyTimer.innerHTML = 'Verification Complete';else {
  ...
  var stopTimeCountdown = function stopTimeCountdown() {
    clearInterval(timer);
  };
};

//after set timeout scroll to accept/denial (randomize verdict)
var scrollToVerdict = function scrollToVerdict() {
  //randomize verdict for demo
  var num = Math.round(Math.random());
  var verdict = num ? 'approval' : 'denial';
  ...
  var vPanel = document.getElementById(verdict);
  vPanel.className = 'panel show';
  ...
  var verdictCardWrapper = document.getElementById('verdict-card-wrapper');
...
var viewRegistryButton = document.getElementById('view-registry-banner');
viewRegistryButton.onclick = function () {
  document.getElementById('apply-btn-wrapper').setAttribute('style', 'height: 0');
  document.getElementById('weeve-registry-inner').removeAttribute('style');
  var statusBlocksCol = document.getElementsByClassName('status-block');
  Array.prototype.forEach.call(statusBlocksCol, function (statusBlock) {
    return statusBlock.setAttribute('style', 'z-index: 0');
  });
  var processWrapperHeight = $('#curve').position().top + $('#curve').outerHeight(true);
  var processWrapper = document.getElementById('apply-process-wrapper');
  processWrapper.setAttribute("style", 'height: ' + processWrapperHeight + 'px;');
  ...
  var typeAllWrapper = document.getElementById('type-all-card-wrapper');
  typeAllWrapper.setAttribute('style', '-webkit-transform: scale(1);transform: scale(1);');
  typeAllWrapper.className += ' current-filter';

  // create curve path
  var centerX = $('.current-filter').position().left + $('.current-filter').outerWidth(true) / 2;
  var filterX = $('#filter-inner').position().left;
  var filterY = ($('#filter-inner').position().top + $('#filter-inner').outerHeight(true)) / 10;
  var filterPath = $('#filter-path');
  var curveWidth = $('#type-all').outerWidth(true) * 2;
  var endX = $('#filter-inner').width();
  createPath(filterX, filterY, centerX, endX, filterY, curveWidth, filterPath);

  var height = window.innerHeight;
  var innerWrapper = document.getElementById('weeve-registry-inner');
  innerWrapper.setAttribute("style", 'top: -' + height + 'px');

  var listTop = $('#filter-curve').position().top + $('#filter-curve').outerHeight(true);
  document.getElementById('registry-list-wrapper').setAttribute('style', 'top: ' + (listTop - height) + 'px');

  var footerHeight = $('#register-device').outerHeight(true);
  var deviceList = document.getElementById('registry-list');
  var deviceListHeight = listTop - height + footerHeight;
...
var filterBlocks = document.getElementsByClassName('filter-card-body-hover');
Array.prototype.forEach.call(filterBlocks, function (filter) {
  filter.onmouseover = function (event) {
    mouseLocation(function (x, y) {
      var filterX = $('#filter-inner').position().left;
      var filterY = ($('#filter-inner').position().top + $('#filter-inner').outerHeight(true)) / 10;
      var filterPath = $('#filter-path');
      var curveWidth = filter.offsetWidth * 2;
      var endX = $('#filter-inner').width();
...
document.getElementById('filter-inner').onmouseout = function (event) {
  updateFilterPath();
};

var updateFilterPath = function updateFilterPath() {
  var statusX = $('#filter-inner').position().left;
  var statusY = ($('#filter-inner').position().top + $('#filter-inner').outerHeight(true)) / 10;
  var statusPath = $('#filter-path');
  var endX = $('#filter-inner').width();
  var centerX = $('.current-filter').position().left + $('.current-filter').outerWidth(true) / 2;
  var curveWidth = $('.filter-type').outerWidth(true) * 2;

  createPath(statusX, statusY, centerX, endX, statusY, curveWidth, statusPath);
};

//add click handlers to filters
var filterEls = document.getElementsByClassName('card-wrapper');
Array.prototype.forEach.call(filterEls, function (filter) {
  var selectedFilterCol = document.getElementsByClassName('current-filter');
  filter.onclick = function (event) {
    Array.prototype.forEach.call(selectedFilterCol, function (selectedFilter) {
...
var filterListBy = function filterListBy(filterId) {
  var deviceList = document.getElementById('registry-list');
  var filteredList = [];
  if (!filterId || filterId === 'type-a') {
    Object.keys(dummyDeviceList).forEach(function (key) {
      filteredList = [].concat(_toConsumableArray(filteredList), _toConsumableArray(dummyDeviceList[key]));
    });
  } else {
